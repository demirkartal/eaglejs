...
class EagleJS extends Array {
   ...
   * @return {EagleJS} A new collection.
   */
  mapPropUntil (name, filter = null, until = null) {
    const $elements = new EagleJS()
    this.forEach((element) => {
   ...
   * @return {EagleJS} A new collection.
   */
  nextUntil (selector = null, filter = null) {
   ...
   * @return {this} The current collection.
   */
  off (events, listener, options = false) {
   ...
   * @return {this} The current collection.
   */
  on (events, listener, options = false) {
    const eventNames = events.match(/\S+/g) || []
    this.forEach((element) => {
      eventNames.forEach((event) => {
   ...
   * @return {EagleJS} A new collection.
   */
  parentsUntil (selector = null, filter = null) {
   ...
   * @return {EagleJS} A new collection.
   */
  prevUntil (selector, filter = null) {
